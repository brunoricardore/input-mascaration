...
dist.unmask = formatter_1 = dist.formatter = void 0;
/**
 *
...
var formatter_1 = dist.formatter = formatter;
/**
 *
...
const InputMask = (props) => {
    ...
    react.exports.useEffect(() => {
        setState(formatter_1(props.mask, props.value));
    }, []);
    react.exports.useEffect(() => {
        const pos = state === null || state === void 0 ? void 0 : state.replace(/\D/g, '').length;
        inputRef.current.setSelectionRange(pos, pos);
    }, [state]);
    const handleChange = (event) => {
        const eventValue = event.target.value;
        const replaceRegex = new RegExp(`${props.maskChar || '#'}`, 'g');
        const rawValue = eventValue.replace(replaceRegex, '');
        console.log(eventValue, rawValue);
        let formattedValue = '';
        if (props.beforeMask) {
            formattedValue = props.beforeMask(rawValue);
        }
        formattedValue = formatter_1(props.mask, rawValue);
        setState(formattedValue);
        props.onValueChanged(formattedValue);
    };
    return React.createElement("input", { ref: inputRef, value: state, onChange: handleChange });
};
